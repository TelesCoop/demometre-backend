# Generated by Django 3.2.11 on 2022-02-16 11:14

import _operator
from django.db import migrations, models
import django.db.models.deletion
import django.utils.timezone
import model_utils.fields
import modelcluster.fields


class Migration(migrations.Migration):

    dependencies = [
        ('open_democracy_back', '0006_auto_20220216_1041'),
    ]

    operations = [
        migrations.CreateModel(
            name='QuestionFilter',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('created', model_utils.fields.AutoCreatedField(default=django.utils.timezone.now, editable=False, verbose_name='created')),
                ('modified', model_utils.fields.AutoLastModifiedField(default=django.utils.timezone.now, editable=False, verbose_name='modified')),
                ('sort_order', models.IntegerField(blank=True, editable=False, null=True)),
                ('intersection_operator', models.CharField(choices=[('and', 'et'), ('or', 'ou'), ('not', 'non')], max_length=8)),
                ('numerical_first_operator', models.CharField(choices=[(_operator.lt, '<'), (_operator.gt, '>'), (_operator.le, '<='), (_operator.ge, '>='), (_operator.ne, '!='), (_operator.eq, '=')], max_length=8)),
                ('numerical_first_value', models.IntegerField(blank=True, null=True)),
                ('numerical_second_operator', models.CharField(choices=[(_operator.lt, '<'), (_operator.gt, '>'), (_operator.le, '<='), (_operator.ge, '>='), (_operator.ne, '!='), (_operator.eq, '=')], max_length=8)),
                ('numerical_second_value', models.IntegerField(blank=True, null=True)),
                ('boolean_response', models.BooleanField(blank=True, null=True)),
                ('conditional_question', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='questions_that_depend_on_me', to='open_democracy_back.question', verbose_name='Filtre par question')),
                ('question', modelcluster.fields.ParentalKey(on_delete=django.db.models.deletion.CASCADE, related_name='question_filters', to='open_democracy_back.question')),
                ('response_choices', models.ManyToManyField(to='open_democracy_back.ResponseChoice')),
            ],
            options={
                'abstract': False,
            },
        ),
        migrations.RemoveField(
            model_name='questionfilterboolean',
            name='questionfilterbase_ptr',
        ),
        migrations.RemoveField(
            model_name='questionfilternumericalorscale',
            name='questionfilterbase_ptr',
        ),
        migrations.RemoveField(
            model_name='questionfilterresponsechoices',
            name='questionfilterbase_ptr',
        ),
        migrations.RemoveField(
            model_name='questionfilterresponsechoices',
            name='response',
        ),
        migrations.DeleteModel(
            name='QuestionFilterBase',
        ),
        migrations.DeleteModel(
            name='QuestionFilterBoolean',
        ),
        migrations.DeleteModel(
            name='QuestionFilterNumericalOrScale',
        ),
        migrations.DeleteModel(
            name='QuestionFilterResponseChoices',
        ),
    ]
